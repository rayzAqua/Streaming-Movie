  ...
  return (
    <>
      <div className="border border-border p-1 hover:scale-95 transitions relative rounded overflow-hidden">
        <div className="w-full" onClick={() => handleClickMovie(movie)}>
          <img
            src={`${movie?.poster}`}
            alt={movie?.title}
        ...
        <div className="absolute flex-btn gap-2 bottom-0 right-0 left-0 bg-main bg-opacity-60 text-white px-4 py-3">
          ...
          {user ? (
            <button
              ...
              className={`h-9 w-9 text-sm flex-colo transitions border-2 border-subMain rounded-md ${
                isFilmFavorite(movie.id)
                  ? "bg-subMain hover:bg-transparent"
                  : "bg-transparent hover:bg-subMain"
              } text-white`}
            >
              <FaHeart />
            </button>
            ...
            <Link
              to={`/login`}
              className="h-9 w-9 text-sm flex-colo transitions border-2 border-subMain rounded-md bg-transparent hover:bg-subMain text-white "
            >
              <FaHeart />
            </Link>
          )}
