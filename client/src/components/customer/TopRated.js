...
function TopRated() {
  ...
  async function getTopRateFilm() {
    const result = await axios.get(
      axios.defaults.baseURL + `/films/topRatedFilms`
    );
                  ...
                  <button
                    ...
                    className={`w-12 h-12 flex-colo transitions ${
                      isFilmFavorite(movie.id)
                        ? "text-subMain bg-opacity-70 hover:text-white hover:bg-opacity-30"
                        : "text-white bg-opacity-30 hover:bg-opacity-70 hover:text-subMain"
