...
function Actor() {
  ...
  useEffect(() => {
    getActors();
    actor.length % 10 == 0
      ? setLastPage(Math.floor(actor.length / 10))
      : setLastPage(Math.floor(actor.length / 10) + 1);
  }, [param, load]);
    ...
    const query =
      form === "add"
        ? await axiosApiInstance.post(
            axiosApiInstance.defaults.baseURL + `/actors/create`,
            payload
          )
        : await axiosApiInstance.put(
            axiosApiInstance.defaults.baseURL + `/actors/edit/${id}`,
            payload
          );
