...
function Payment() {
  ...
  const currentPayment = payments.slice(
    indexOfFirstPayment,
    indexOfLastPayment
  );
  ...
  async function getPayment() {
    const result = await axiosApiInstance.get(
      axiosApiInstance.defaults.baseURL + `/payment/getAll`
    );
  ...
  useEffect(() => {
    getPayment();
    payments.length % 10 == 0
      ? setLastPage(Math.floor(payments.length / 10))
      : setLastPage(Math.floor(payments.length / 10) + 1);
  }, [param, load]);
              ...
              <tbody className="bg-main divide-y divide-gray-800">
                {payments.map((payment, index) => (
                  <tr key={payment.id}>
                    ...
                    <td className={`${Text}`}>
                      {payment.film_name == null
                        ? payment.pricing_name
                        : payment.film_name}
                    </td>
                    <td className={`${Text}`}>
                      {payment.pay.toLocaleString("VN-vi")} VND
                    </td>
                    <td className={`${Text}`}>
                    ...
                    {/* <td className={`${Text} flex-rows gap-2 mt-2`}>
                      <button
                        ...
                        className={`border border-white ${
                          payment.status == 0 ? "bg-subMain " : "bg-green-400"
                        } flex-rows gap-2 text-white rounded py-1 px-2`}
                      >
                        {!payment.status == 1 ? (
