...
function Pricing() {
  ...
  async function getPricing() {
    const result = await axiosApiInstance.get(
      axiosApiInstance.defaults.baseURL + `/pricing/getAll`
    );
    setLoad(true);
  ...
  useEffect(() => {
    getPricing();
    pricing.length % 10 == 0
      ? setLastPage(Math.floor(pricing.length / 10))
      : setLastPage(Math.floor(pricing.length / 10) + 1);
  }, [param, load]);
    ...
    const query =
      form === "add"
        ? await axiosApiInstance.post(
            axiosApiInstance.defaults.baseURL + `/pricing/create`,
            payload
          )
        : await axiosApiInstance.put(
            axiosApiInstance.defaults.baseURL + `/pricing/edit/${id}`,
            payload
          );
    if (query?.status === 200 || query?.status === 201) {
      toast.success(query?.data.msg);
      console.log(query);
    } else toast.error(query?.data?.message + "! Please try again");
              ...
              <tbody className="bg-main divide-y divide-gray-800">
                {currentPricing.map((p, index) => (
                  <tr key={p.id}>
                    ...
                    <td className={`${Text}`}>
                      {p.price.toLocaleString("VN-vi")} VND
                    </td>
