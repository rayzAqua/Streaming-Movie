...
function MoviesPage() {
  ...
  async function getActiveFilm() {
    try {
      ...
      const result = await axios.get(
        axios.defaults.baseURL + `/films/search${query}`
      );
  ...
  const handleFilter = async () => {
    ...
    if (genres.value !== 0) {
      if (searchFilter != "") query += `&genre=${genres.value}`;
      else query += `genre=${genres.value}`;
    }
    if (rates.value !== 0) {
      if (searchFilter != "" || genres.value !== 0)
        query += `&rate=${rates.value}`;
      else query += `rate=${rates.value}`;
    }
    // console.log(query);
    // console.log(genres);
    console.log(rates);
